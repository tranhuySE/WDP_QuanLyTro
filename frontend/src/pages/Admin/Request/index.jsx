...
import { Ban, ClipboardCheck, Eye, Check } from 'lucide-react';
...
const RequestManagement = () => {
    ...
    const AccessRequest = async (original) => {
        try {
            const res = await RequestAPI.changeRequestStatus({
                requestId: original?._id,
                status: 'CONFIRM',
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                header: 'Người tạo',
                size: 100,
                Cell: ({ cell }) => <div>{cell.getValue()?.fullname}</div>,
            },
            {
                accessorKey: 'room',
                header: 'Số phòng',
            ...
            Cell: ({ row }) => (
                <Space Space size="small">
                    ...
                    <Button
                        ...
                        size="sm"
                        disabled={
                            (role === 'admin' && row.original.status !== 'PENDING') ||
                            (role === 'staff' && row.original.status !== 'ASSIGNED')
                        }
                        onClick={() => {
                            if (role === 'admin') {
                    ...
                    <Button
                        ...
                        disabled={row.original.status !== 'PENDING'}
                        size="sm"
                        onClick={() => AccessRequest(row.original)}
                    >
                        <Check size={16} />
                    </Button>
                </Space>
